...
def plot_prec_recall(alg_name, recall, precision, result_dir, nms_prob=None):
    # average precision
    ...
    plt.grid(1)
    #plt.show()
    plt.savefig(result_dir + "plot_prec_recall.png")
...
def plot_spec(op_file_name, ip_file, gt_pos, nms_pos, nms_prob, y_prediction, params, save_ims):
    ...
    nms_pos_norm = np.array(nms_pos_norm)
    nms_prob = np.array([x[0] for x in nms_prob])
