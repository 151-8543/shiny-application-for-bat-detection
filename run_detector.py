...
def run_detector(det, audio, file_dur, samp_rate, detection_thresh, do_time_expansion):
    ...
    """
async def main (detection_thresh, do_time_expansion, save_res, data_dir, op_ann_dir, op_file_name_total, model_file):
    #load the model
    ...
    for file_cnt, file_name in enumerate(audio_files):
        ...
        if save_res:
            ...
            pred_classes = np.zeros((len(det_time), 1), dtype=np.int32)
            ...
            if num_calls > 0:
                res = {'filename':file_name_root, 'time':det_time,
                      ...
                      'pred_prob':pred_prob}
                #res = {'Filename':file_name_root,
                #       'Number of Bat calls': num_calls}
                results.append(res)
